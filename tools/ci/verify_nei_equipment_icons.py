#!/usr/bin/env python3
"""Fail an Android build if its generated support archive omits NEI icons."""
...
def main() -> int:
    ...
    if missing:
        print("generated support archive is missing NEI equipment icons:", file=sys.stderr)
        for icon in missing:
            print(f"  {icon}", file=sys.stderr)
        return 1

    print(f"verified {len(REQUIRED_ICONS)} NEI equipment icons in {archive_path}")
    return 0
