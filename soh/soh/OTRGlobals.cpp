...
void OTRGlobals::Initialize() {
    ...
    // NEI folder: scan for .o2r/.zip archives (e.g. garo.o2r generated by
    // tools/glb_to_o2r.py) and add them to the ArchiveManager.
    // .pak files in this folder are handled separately by pak_loader.
    std::string neiPath = Ship::Context::LocateFileAcrossAppDirs("nei", appShortName);
    if (neiPath.empty() || !std::filesystem::exists(neiPath)) {
        neiPath = Ship::Context::GetPathRelativeToAppDirectory("nei", appShortName);
    }
    if (std::filesystem::exists(neiPath) && std::filesystem::is_directory(neiPath)) {
        for (const auto& entry : std::filesystem::directory_iterator(neiPath)) {
